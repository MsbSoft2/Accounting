...
namespace Accounting.App
{
    public partial class frmNewAccounting : Form
    {
        ...
        {
            db = new UnitOfWork();
            dgvCostomers.AutoGenerateColumns = false;
            dgvCostomers.DataSource = db.CustomerRepository.GetNameCustomers();
            if (AccountID != 0)
            {
        ...
        {
            dgvCostomers.AutoGenerateColumns = false;
            dgvCostomers.DataSource = db.CustomerRepository.GetNameCustomers(txtFilter.Text);
        }

        private void dgvCostomers_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            txtName.Text = dgvCostomers.CurrentRow.Cells[0].Value.ToString();
        }
